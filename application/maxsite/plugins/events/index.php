...
function events_widget_form($num = 1) 
{
	...
	$form .= mso_widget_create_form('Формат даты', form_input( array( 'name'=>$widget . 'format_date', 'value'=>$options['format_date'] ) ), 'Как это <a href="http://ru.php.net/date" target="_blank">принято в PHP</a>');
	
	$form .= mso_widget_create_form('События', form_textarea( array( 'name'=>$widget . 'events', 'value'=>$options['events'] ) ), 'Указывайте по одному событию в каждом абзаце в формате:<br><strong>дата | до | после | текст события</strong><br><strong>дата</strong> в формате yyyy-mm-dd<br><strong>до</strong> - выводить событие до наступления N-дней<br><strong>после</strong> - выводить событие после прошествия N-дней<br><strong>В тексте события</strong> можно использовать HTML<br><strong>ПРИМЕР:</strong> 2008-09-01 | 3 | 1 | Пора в школу!');
